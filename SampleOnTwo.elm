...
init = { currentValue = False, numUpdates = 0 }
...
view m =
  div [] [ p [] [text ("Num updates: " ++ (toString m.numUpdates))]
...
type alias Inputs = { dt : Float, space : Bool }


recordedInputs = Signal.map2 (\dt space -> { dt = dt, space = space }) timer inputs
...
main = Signal.map view <| Signal.foldp (\x -> update x.space) init (Signal.sampleOn timer recordedInputs)
