...
update action model =
    case action of
        Tick dt ->
            updateTicks { dt = dt } model

        Wasd wasd ->
            updateKeys wasd model

        Input inputs ->
            updateTicks inputs (updateKeys inputs model)
...
view model =
    ...
    div []
        [ h1 [] [ text "Inputs" ]
        , ul []
            [ li [] [ text ("Number of times W has been pressed: " ++ (toString model.numW)) ]
...
main =
    map view <| foldp update init inputs
