...
yWall : Edge                               
yWall = Wall (Painted Color.yellow)


rWall : Edge
rWall = Wall (Painted Color.red)
...
world =
  ...
  , floorTiles = [ BlueFloor, BlueFloor,  BlueFloor,  BlueFloor,  BlueFloor
                 , BlueFloor, RedFloor, RedFloor, RedFloor, BlueFloor
                 , BlueFloor, RedFloor, RedFloor, RedFloor, BlueFloor
                 , BlueFloor, RedFloor, RedFloor, RedFloor, BlueFloor
                 , BlueFloor, BlueFloor,  RedFloor,  BlueFloor,  BlueFloor ]
  , horizontalEdges = [ yWall, yWall, yWall, yWall, yWall  -- y is constant
                      , Air,   Air,   Air,   Air,   Air
                      , Air,   Air,   Air,   Air,   Air
                      , Air,   Air,   Air,   Air,   Air
                      , Air,   Air,   Air,   Air,   Air                           
                      , yWall, yWall, yWall, yWall, yWall
                      ]
  , verticalEdges = [ rWall, Air, Air, Air, Air, rWall  -- x is constant
                    , rWall, Air, Air, Air, Air, rWall
                    , rWall, Air, Air, Air, Air, rWall
                    , rWall, Air, Air, Air, Air, rWall
                    , rWall, Air, Air, Air, Air, rWall
                    ]
...
input : Signal Input         
input = Signal.sampleOn ticker (Signal.map Wasd Keyboard.wasd)
...
positionTile p f =
  let
    (wx, wy) = world.size
    halfTile = worldView.tileWidth / 2.0
    centreOffset = (((toFloat wx) * worldView.tileWidth) / 2.0 - halfTile)
    x = (toFloat (p % wx)) * worldView.tileWidth
    y = (toFloat (p // wx)) * worldView.tileHeight
  in
...
drawEdge e =
  case e of
    Air -> filled Color.white (rect 1 worldView.tileHeight)
    Wall m -> drawWallEdge m


drawWallEdge : Material -> Form
drawWallEdge m =
  case m of
    Painted c -> filled c (rect 1 worldView.tileHeight)
    -- TODO: Texturing
    Wallpapered t -> filled Color.black (rect 1 worldView.tileHeight)

                     
drawHorizontalEdge : Int -> Edge -> Form
drawHorizontalEdge p e =
  let
    (numTilesX, numTilesY) = world.size
    numEdgesX = toFloat (numTilesX + 1)
    numEdgesY = toFloat (numTilesY + 1)

    halfWorldWidth = (worldView.tileWidth * (toFloat numTilesX) / 2.0)
    halfWorldHeight = (worldView.tileHeight * (toFloat numTilesY) / 2.0)
    halfTileWidth = worldView.tileWidth / 2.0
    halfTileHeight = worldView.tileHeight / 2.0

    x = (toFloat (p % numTilesX)) * worldView.tileWidth - halfWorldWidth + halfTileWidth
    y = (toFloat (p // numTilesX)) * worldView.tileHeight - halfWorldHeight
    f = rotate (degrees 90) (drawEdge e)
...
drawVerticalEdge p e =
  let
    ...
    x = (toFloat (p % (round numEdgesX))) * worldView.tileWidth - halfWorldWidth
    y = (toFloat (p // (round numEdgesX))) * worldView.tileHeight - halfWorldHeight + halfTileHeight
    f = drawEdge e
  in
    move (x, y) f
...
raycast position lookAngle angle =
  let
    ...
    lookingUp = angle > 0.0 && angle < (degrees 180)
    lookingLeft = angle > (degrees 90) && angle < (degrees 270)
    ...
    beta = if angle > lookAngle then angle - lookAngle else lookAngle - angle
    correctedDistance = fisheyeDistance * (cos beta)
  in
    correctedDistance
